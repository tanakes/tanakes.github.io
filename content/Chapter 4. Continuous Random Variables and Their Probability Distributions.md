...
## 📊 The Three Cases for npq

| Condition          | What to Do                                                       | Formula to Use                                                                                                     |
| ------------------ | ---------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------ |
| **npq > 9** ✅      | Normal approximation **without** continuity correction           | $$\boxed{P(a \leq x \leq b) = P\left(\frac{a-np}{\sqrt{npq}} \leq Z \leq \frac{b-np}{\sqrt{npq}}\right)}$$         |
| **5 < npq ≤ 9** ⚠️ | Normal approximation **with** continuity correction              | $$\boxed{P(a \leq x \leq b) = P\left(\frac{a-0.5-np}{\sqrt{npq}} \leq Z \leq \frac{b+0.5-np}{\sqrt{npq}}\right)}$$ |
| **npq ≤ 5** ❌      | **Do NOT use normal approximation** — use exact binomial formula | $$P(x) = \binom{n}{x} p^x q^{n-x}$$                                                                                |
...
**Setup**: n = 20, p = 0.3 → q = 0.7 → npq = 20 × 0.3 × 0.7 = **4.2**

### Case 1: npq = 4.2 ≤ 5 → Use Exact Binomial
**Question**: P(x = 6)?

$$P(x=6) = \binom{20}{6} (0.3)^6 (0.7)^{14} = 38760 × 0.000729 × 0.00678 ≈ \boxed{0.1916}$$

✅ No normal approximation allowed here!

---
### Case 2: npq = 6.3 → 5 < npq ≤ 9! Use continuity correction

**Setup**: n = 30 trials, p = 0.7 → q = 1 − p = 0.3

**First, verify which case applies**:
$$npq = 30 \times 0.7 \times 0.3 = 30 \times 0.21 = \boxed{6.3}$$

✓ Since **5 < 6.3 ≤ 9**, we **must use continuity correction** (±0.5)

**Question**: P(19 < x ≤ 23)?

**Step 1**: μ = np = 21, σ = √(npq) = √6.3 ≈ 2.510

**Step 2**: Apply ±0.5 correction:
- Lower: 19 − 0.5 = **18.5**
- Upper: x ≤ 23 → 23 + 0.5 = **23.5**

**Step 3**: Convert to Z:
$$Z_{lower} = \frac{18.5 - 21}{2.510} = -1, \quad Z_{upper} = \frac{23.5 - 21}{2.510} = 1.00$$

**Step 4**: Use Z-table:
$$P(-1 \leq Z \leq 1.00) = F(1.00) - F(-1) = 0.8413 - 0.1587 = \boxed{0.6826}$$
---

**Setup**: n = 50, p = 0.5 → npq = 50 × 0.5 × 0.5 = **12.5 > 9**

### Case 3: npq = 12.5 > 9 → Normal approx WITHOUT correction
**Question**: P(20 ≤ x ≤ 30)?

**Step 1**: μ = 25, σ = √12.5 ≈ 3.536

**Step 2**: NO ±0.5 correction! Use raw values:
$$Z_{lower} = \frac{20 - 25}{3.536} = -1.41, \quad Z_{upper} = \frac{30 - 25}{3.536} = 1.41$$

**Step 3**: Z-table:
$$P(-1.41 \leq Z \leq 1.41) = 0.9207 - 0.0793 = \boxed{0.8414}$$
